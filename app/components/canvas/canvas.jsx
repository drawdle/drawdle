	...
	render() {
		return (
			<>
				...
				<div
					id="toolbar"
					className="fixed flex justify-start items-center gap-2 bg-beige-800 shadow-md p-2 rounded-md text-beige-200"
					style={{
						left: clamp(
							this.state.toolbarPos.x,
							4,
				...
				>
					...
					<div className="border-beige-700 border-l w-0 h-8"></div>
					{[
						{
							text: "Zoom in",
							icon: "bi-zoom-in",
						...
						<>
							...
							{this.state.tool != "Color Picker" && (
								<div className="relative">
									...
									>
										<div className="flex flex-col justify-center items-center gap-px w-8 h-8">
											<div className="flex justify-center items-center w-4 h-4">
												<div
													className="bg-beige-200 rounded-full"
													style={{
														width: `${
															(clamp(
																this.state.tool == "Erase"
											...
											<p className="w-8 h-2 text-[8px] align-middle">
												{this.state.tool == "Erase"
													? this.state.eraserSize
													: this.state.brushSize}
											</p>
										</div>
										<i className="w-4 h-2 [&::before]:text-xs [&::before]:-translate-y-2 bi-caret-down-fill"></i>
									</button>
									<BrushMenu
...
function BrushMenu(props) {
	return (
		<>
			{props.visible && (
				<div className="top-0 left-0 absolute flex flex-col gap-2 bg-beige-800 p-3 rounded-md w-64 h-48 translate-y-11">
					<p className="-mb-1 text-beige-600 text-sm">Size</p>
					<div className="flex flex-row justify-center items-center gap-2">
						...
						<Slider
							...
							handleRender={(node) => {
								return (
									<div
										className="bg-beige-700 rounded-full w-[14px] h-[14px] transition"
										style={{
											...node.props.style,
											top: "0px",
											position: "absolute",
					...
					<p className="-mb-1 text-beige-600 text-sm">Opacity</p>
					<div className="flex flex-row justify-center items-center gap-2">
						<input
							className="border-[#fff2] hover:border-[#fff4] focus:border-[#fff4] bg-beige-700 px-1 py-3 border rounded-md w-10 h-4 text-sm outline-none [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
